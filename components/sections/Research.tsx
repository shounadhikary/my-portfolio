...
export function Research() {
  return (
    <section id="research" className="py-24 bg-[#050810]">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {research.map((item, i) => (
            ...
            >
              ...
              <h3 className="font-display font-bold text-slate-100 text-base leading-snug mb-2">{item.title}</h3>
              ...
              <div className="pt-3 border-t border-[#1E2D3D] flex items-center justify-between">
                <span className="text-xs text-slate-500">{item.papers} paper{item.papers !== 1 ? "s" : ""}</span>
                <div className="flex gap-1">
                  {[0, 1, 2].map((d) => (
                    <div key={d} className="w-1.5 h-1.5 rounded-full"
                      style={{ background: d < item.papers ? item.color : "#1A2332" }} />
                  ))}
                </div>
              </div>
            </motion.div>
